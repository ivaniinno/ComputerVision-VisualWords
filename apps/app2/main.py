...
ARCHIVE_DIR = Path("/Users/rail/Downloads/archive")
...
app = FastAPI(title="BoVW Retrieval App v2 (images)")
...
def load_data() -> None:
    ...
    FILENAME_TO_IDX.update({str(fn): i for i, fn in enumerate(FILENAMES)})
...
async def search_by_filename(payload: Dict[str, Any]):
    ...
    if "hits" in res:
        ...
        # Add image_url using the /archive mount
        merged["image_url"] = "/archive/" + merged["filename"].astype(str)
...
async def search_by_upload(file: UploadFile = File(...), top_k: int = 12):
    ...
    hist_counts, hist_edges = np.histogram(sims, bins=40, range=(-0.1, 1.0))
    uid = str(uuid.uuid4())
    LAST_UPLOADS[uid] = {"sims": sims, "edges": hist_edges, "q": q.astype(np.float32)}
    if len(LAST_UPLOADS) > 5:
        for k in list(LAST_UPLOADS.keys())[:-5]:
            LAST_UPLOADS.pop(k, None)
    hits = [
        {"index": int(j), "filename": str(FILENAMES[j]), "similarity": float(sims[j])}
        for j in idx
    ]
    ...
    merged = df_hits.merge(meta, on="filename", how="left")
    merged["image_url"] = "/archive/" + merged["filename"].astype(str)
    merged = df_to_records_safe(merged)

    return {"upload_id": uid, "hits": merged, "hist": {"counts": hist_counts.tolist(), "edges": hist_edges.tolist()}}
...
async def bin_items(payload: Dict[str, Any]):
    ...
    df_items["image_url"] = "/archive/" + df_items["filename"].astype(str)
...
async def dataset_list(author: Optional[str] = None, genres: Optional[str] = None, page: int = 1, size: int = 24, query: Optional[str] = None):
    ...
    rows["image_url"] = "/archive/" + rows["filename"].astype(str)
...
async def metadata(filename: str):
    ...
    rr["image_url"] = f"/archive/{rr['filename']}"
